...
// Max display dimensions for question images (matches Word export limits)
const HTML_IMG_MAX_W = 400;
const HTML_IMG_MAX_H = 300;
...
/**
 * Load an image from the uploads directory and return a base64 data URI.
 * Mirrors loadImageForDocx in printDocx.js — reads from disk, no HTTP needed.
 * Returns null on any failure (missing file, unsupported format, read error).
 */
function loadImageAsDataUri(imgUrl, uploadsDir) {
...
/**
 * Resolve the value to use in an HTML src="" attribute for an image.
 *
 * Primary path: read from disk and return a self-contained data URI so the
 * browser never makes a network request for the image (works behind any
 * load balancer or reverse proxy).
 *
 * Fallback: build an absolute URL from PUBLIC_URL + image path, e.g.
 * http://20.110.8.201:5000/uploads/image.jpg — so the browser can always
 * fetch the image regardless of how the page was opened (blob: URL, iframe, etc.).
 */
function resolveImgSrc(url, uploadsDir, baseUrl) {
  if (!url) return '';
  if (uploadsDir) {
    const dataUri = loadImageAsDataUri(url, uploadsDir);
    if (dataUri) return dataUri; // base64 chars are HTML-safe; no esc() needed
  }
  // Already an absolute URL — return as-is.
  if (/^https?:\/\//.test(url)) return esc(url);
  // Prepend PUBLIC_URL to form e.g. http://20.110.8.201:5000/uploads/image.jpg
  const base = (baseUrl || '').replace(/\/$/, '');
  return esc(base + (url.startsWith('/') ? url : '/' + url));
}
...
/**
 * Render one question as an HTML string.
 *
 * Order matches Word export: prompt text → image (if any) → answer choices.
 *
 * @param {object} q             Question from the snapshot payload.
 * @param {number} num           1-based question number.
 * @param {boolean} showCorrect
 * @param {string}  [uploadsDir] Absolute path to uploads dir; enables data-URI embedding.
 * @param {string}  [baseUrl]    PUBLIC_URL used to build absolute fallback src values.
 */
function renderQuestion(q, num, showCorrect, uploadsDir, baseUrl) {
  // ── 1. Prompt image (resolved to data URI or absolute URL) ────────────────
  const promptSrc = resolveImgSrc(q.prompt_image, uploadsDir, baseUrl);
  const promptImg = promptSrc
    ? `<div class="q-img-wrap"><img src="${promptSrc}" class="q-img" alt=""></div>`
    : '';

  // ── 2. Answer choices / blank area ────────────────────────────────────────
  let bodyHtml = '';
  ...
  if (q.type === 'multiple_choice') {
    ...
    bodyHtml = `<div class="mc-options">${opts.map((opt, i) => {
      ...
      const optSrc  = resolveImgSrc(opt.image, uploadsDir, baseUrl);
      const img     = optSrc
        ? `<img src="${optSrc}" class="opt-img" alt="">`
        : '';
    ...
    bodyHtml = `<div class="dd-items">${items.map((item, idx) => {
      const itemSrc = resolveImgSrc(item.item_image, uploadsDir, baseUrl);
      const img     = itemSrc
        ? `<img src="${itemSrc}" class="opt-img" alt="">`
        : '';
...
const BASE_CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Question image: matches Word export max 400×300, scales within page */
  .q-img-wrap { margin: 4px 0 10px; }
  .q-img { max-width: min(400px, 100%); max-height: 300px; display: block; object-fit: contain; }
  ...
  @media print {
    ...
    .q-img-wrap { page-break-inside: avoid; }
  }
...
/**
 * Build a complete print HTML document.
 *
 * @param {object} payload      Stored print_job_versions.payload.
 * @param {string} mode         'test' | 'key' | 'answers_only'
 * @param {string} [baseUrl]    PUBLIC_URL (e.g. 'http://20.110.8.201:5000').
 *                              Prepended to image paths to form absolute URLs,
 *                              e.g. http://20.110.8.201:5000/uploads/image.jpg.
 * @param {string} [uploadsDir] Absolute path to the uploads directory on disk.
 *                              When provided, images are embedded as data URIs —
 *                              no network requests, works behind any load balancer.
 */
function buildPrintHtml(payload, mode, baseUrl, uploadsDir) {
...
${rows}
  ...
  const titleImgSrc = resolveImgSrc(test.title_image, uploadsDir, baseUrl);
  const titleImg    = titleImgSrc
    ? `<img src="${titleImgSrc}" class="title-img" alt="">`
    : '';
  const questionsHtml = (questions || [])
    .map((q, i) => renderQuestion(q, i + 1, showCorrect, uploadsDir, baseUrl))
    .join('\n');
